-- Mart model: one row per policy with current status derived from events.
-- Status logic: last event determines status.
--   PolicyIssued   → ACTIVE
--   PolicyCancelled → CANCELLED
--   PolicyChanged   → ACTIVE (financial change, policy remains active)
