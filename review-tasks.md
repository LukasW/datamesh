...
- [x] Debezium-Connector für Partner (`infra/debezium/partner-outbox-connector.json`) auf `io.confluent.connect.avro.AvroConverter` umstellen
- [x] Debezium-Connector für Product (`infra/debezium/product-outbox-connector.json`) analog umstellen
- [x] Avro-Schemas für alle Partner-Events als `.avsc`-Dateien anlegen (→ TASK-03)
- [x] `schema.registry.url` in beiden Connector-Configs setzen
...
- [x] `PersonFlywayMigrationIT.java`: Flyway läuft durch; alle Tabellen und Indizes vorhanden
- [x] `PersonOutboxRoundtripIT.java`: Person anlegen → Outbox-Eintrag in DB → Debezium-Mock oder direkter Kafka-Consumer-Check
- [x] `PersonRestAdapterIT.java`: POST/GET/PUT/DELETE gegen echte DB

**Product-Service** – `product/src/test/java/ch/css/product/integration/`:
- [x] `ProductFlywayMigrationIT.java`
- [x] `ProductOutboxRoundtripIT.java`

**Policy-Service** – `policy/src/test/java/ch/css/policy/integration/`:
- [x] `PolicyFlywayMigrationIT.java`
- [x] `PolicyActivationIT.java`: Policy anlegen → aktivieren → Outbox-Eintrag vorhanden (nach TASK-01)
- [x] `PartnerViewMaterializationIT.java`: `person.v1.created`-Event einspielen → `PartnerView`-Tabelle befüllt
- [x] `ProductViewMaterializationIT.java`: `product.v1.defined`-Event einspielen → `ProductView`-Tabelle befüllt

**Konfiguration:**
- [x] `@QuarkusIntegrationTest` mit `@QuarkusTestResource(PostgreSQLTestResource.class)` und `@QuarkusTestResource(KafkaTestResource.class)` konfigurieren
- [x] Maven-Profil `integration` in allen `pom.xml` Dateien konfigurieren (bereits in CLAUDE.md beschrieben)
...
**Policy-Service:**
- [x] [PartnerEventConsumer.java](policy/src/main/java/ch/css/policy/infrastructure/messaging/PartnerEventConsumer.java): `failure-strategy=dead-letter-queue` konfigurieren; Try/Catch mit strukturiertem Error-Log
- [x] [ProductEventConsumer.java](policy/src/main/java/ch/css/policy/infrastructure/messaging/ProductEventConsumer.java): analog
- [x] [policy/src/main/resources/application.properties](policy/src/main/resources/application.properties) ergänzen:
  ```properties
  ...
  # ... weitere Channels
  ```
- [x] DLQ-Topics in `kafka-init` in [docker-compose.yaml](docker-compose.yaml) vorab anlegen
...
- [x] [PolicyApplicationService.java](policy/src/main/java/ch/css/policy/domain/service/PolicyApplicationService.java) aufteilen in:
...
- [x] `PersonApplicationService.java` aufteilen in `PersonCommandService` + `PersonQueryService`
- [x] `ProductApplicationService.java` aufteilen in `ProductCommandService` + `ProductQueryService`
- [x] Alle Ports in `domain/port/in/` in Command-Ports und Query-Ports aufteilen (falls Use-Case-Ports vorhanden)
- [x] REST-Adapter und UI-Controller auf die aufgeteilten Services umstellen
- [x] Unit-Tests entsprechend anpassen
...
**Partner-Service:**
- [x] [OutboxEvent.java](partner/src/main/java/ch/css/partner/domain/model/OutboxEvent.java) von `domain/model/` nach `infrastructure/messaging/outbox/` verschieben
- [x] [PersonEventPayloadBuilder.java](partner/src/main/java/ch/css/partner/domain/service/PersonEventPayloadBuilder.java) von `domain/service/` nach `infrastructure/messaging/` verschieben
- [x] [OutboxRepository.java](partner/src/main/java/ch/css/partner/domain/port/out/OutboxRepository.java): Port-Interface bleibt in `domain/port/out/`, verweist aber auf einen technischen Typ – dokumentieren oder durch typisierte Domain-Events ersetzen (→ TASK-13)

**Product-Service:**
- [x] `OutboxEvent.java` analog verschieben
- [x] `ProductEventPayloadBuilder.java` analog verschieben

**Policy-Service:**
- [x] Nach TASK-01: neues `OutboxEvent` direkt in `infrastructure/messaging/outbox/` platzieren
...
- [x] `product.v1.state` compacted Topic in `kafka-init` in [docker-compose.yaml](docker-compose.yaml) anlegen (analog `person.v1.state`)
- [x] [ProductApplicationService.java](product/src/main/java/ch/css/product/domain/service/ProductApplicationService.java): Bei jeder Mutation zusätzlich ein State-Event in die Outbox schreiben (Topic: `product.v1.state`)
- [x] ODC-Contract `product.v1.state.odcontract.yaml` in [product/src/main/resources/contracts/](product/src/main/resources/contracts/) anlegen
- [x] [PartnerEventConsumer.java](policy/src/main/java/ch/css/policy/infrastructure/messaging/PartnerEventConsumer.java): Startup-Logik implementieren – zuerst `person.v1.state` von `earliest` konsumieren, dann auf Event-Topics wechseln
- [x] [ProductEventConsumer.java](policy/src/main/java/ch/css/policy/infrastructure/messaging/ProductEventConsumer.java): analog mit `product.v1.state`
- [x] Bootstrap-Protokoll in [partner/specs/business_spec.md](partner/specs/business_spec.md) und [product/specs/business_spec.md](product/specs/business_spec.md) dokumentieren
...
- [x] Keycloak-Service in [docker-compose.yaml](docker-compose.yaml) ergänzen (Image: `quay.io/keycloak/keycloak:24`)
- [x] Realm-JSON `css-realm.json` erstellen: Realm `css`, Clients `partner-service`, `product-service`, `policy-service`, Rollen `UNDERWRITER`, `CLAIMS_AGENT`, `BROKER`, `ADMIN`
- [x] Keycloak-Init-Job in docker-compose: importiert Realm via `--import-realm`
- [x] Quarkus-OIDC-Extension in alle drei `pom.xml` aufnehmen (bereits in CLAUDE.md erwähnt): `quarkus-oidc`
- [x] `application.yml` aller Services: OIDC-Config für Dev- und Prod-Profil eintragen
- [x] `@RolesAllowed`-Annotationen in Application Services ergänzen (gemäss CLAUDE.md)
- [x] Test-User für jeden Rolle in Realm-JSON anlegen
...
- [x] `dbt`-Service aus [docker-compose.yaml](docker-compose.yaml) entfernen (oder in `profiles: [tools]` verschieben)
- [x] Airflow-DAG `dbt_transform_dag.py` in [infra/airflow/dags/](infra/airflow/dags/) anlegen:
...
- [x] Bestehende DAGs in [infra/airflow/dags/](infra/airflow/dags/) auf dbt-DAG als Upstream verweisen
...
**Partner-Service:**
- [x] `domain/model/events/`-Verzeichnis anlegen
- [x] Records erstellen: `PersonCreatedEvent`, `PersonUpdatedEvent`, `PersonDeletedEvent`, `AddressAddedEvent`, `AddressUpdatedEvent`
- [x] [PersonApplicationService.java](partner/src/main/java/ch/css/partner/domain/service/PersonApplicationService.java): Statt `outboxRepository.save(new OutboxEvent(...))` ein typisiertes Event emittieren
- [x] [PersonEventPayloadBuilder.java](partner/src/main/java/ch/css/partner/domain/service/PersonEventPayloadBuilder.java) (nach TASK-09 in Infrastructure): übersetzt `PersonCreatedEvent` → JSON-Payload für Outbox

**Policy-Service:**
- [x] `PolicyIssuedEvent`, `PolicyCancelledEvent`, `PolicyChangedEvent`, `CoverageAddedEvent`, `CoverageRemovedEvent` als Records anlegen

**Product-Service:**
- [x] `ProductDefinedEvent`, `ProductUpdatedEvent`, `ProductDeprecatedEvent` anlegen
...
- [x] `claims/`-Verzeichnis und Maven-Modul anlegen; in [pom.xml](pom.xml) als `<module>claims</module>` eintragen
- [x] `claims/specs/business_spec.md` erstellen (Schadenmeldung, FNOL, Regulierung)
- [x] Minimales Quarkus-Projekt in `claims/` aufbauen (Skelett mit pom.xml, application.yml, Port 9083)
- [x] `CoverageCheckPort` (out-Port) als Interface definieren
- [x] `PolicyCoverageRestClient.java` als REST-Client mit `@RegisterRestClient` und `@CircuitBreaker` implementieren
- [x] Minimaler `ClaimApplicationService` mit `openClaim()` → ruft `CoverageCheckPort` auf
- [x] Kafka-Topics `claims.v1.opened` und `claims.v1.settled` definieren
- [x] ODC-Contracts `claims.v1.opened.odcontract.yaml` und `claims.v1.settled.odcontract.yaml` anlegen
- [x] `claims-service` in [docker-compose.yaml](docker-compose.yaml) ergänzen (Port 9083)
...
- [x] `PageRequest`-Record erstellen: `record PageRequest(int page, int size) {}`
- [x] `PageResult<T>`-Record erstellen: `record PageResult<T>(List<T> content, long totalElements, int totalPages) {}`
- [x] `PersonRepository`-Port: `search()`-Methode auf `PageResult<Person>` umstellen
- [x] `ProductRepository`-Port: analog
- [x] `PolicyRepository`-Port: analog
- [x] JPA-Adapter: `Panache`- oder `TypedQuery.setFirstResult/setMaxResults`-Paginierung implementieren
- [x] REST-Adapter: `?page=0&size=20`-Query-Parameter exponieren
- [x] Qute-Templates: Paginierungs-Controls (Vor/Zurück-Buttons) ergänzen (Text Deutsch: «Weiter», «Zurück»)
...
- [x] [pom.xml](pom.xml) (Parent): `<java.version>25</java.version>` → `<java.version>21</java.version>`
- [x] Alle drei Service-`pom.xml` prüfen: keine eigene Java-Version-Überschreibung
- [x] `application.yml` aller Services: `quarkus.virtual-threads.enabled=true` setzen (Quarkus-Loom-Integration)
- [x] `mvn clean package -DskipTests` in allen Services erfolgreich
...
- [x] [policy/src/main/resources/application.properties](policy/src/main/resources/application.properties): Prod-Profil-Variablen ohne Fallback:
  ```properties
  ...
  ```
- [x] Partner- und Product-`application.yml`: Datenbankverbindung im Prod-Profil ohne Default
- [x] `.env.example` mit allen Pflicht-Variablen und Beschreibungen aktualisieren
- [x] README: Sektion «Required Environment Variables» ergänzen
...
- [x] `PartnerId`, `ProductId`, `PolicyId`, `PersonId` als Records in der jeweiligen `domain/model/`-Schicht anlegen
- [x] Aggregat-Konstruktoren und Methoden-Signaturen auf typisierte IDs umstellen
- [x] Cross-Domain-Referenzen in Policy (`partnerId`, `productId`) auf Typen umstellen
- [x] JPA-Adapter: `AttributeConverter<PartnerId, String>` implementieren
...
- [x] DB-Sequenz in Flyway-Migration anlegen:
  ```sql
  CREATE SEQUENCE policy_number_seq START 1 INCREMENT 1;
  ```
- [x] `PolicyNumberGenerator`-Service in `infrastructure/persistence/` implementieren (kapselt Sequenzabfrage)
- [x] Port `PolicyNumberPort` in `domain/port/out/` definieren
- [x] [PolicyApplicationService.java](policy/src/main/java/ch/css/policy/domain/service/PolicyApplicationService.java) verwendet Port, nicht direkt DB
...
- [x] `infrastructure/messaging/acl/`-Verzeichnis im Policy-Service anlegen
- [x] `PartnerEventTranslator.java`: übersetzt `PersonCreatedPayload` → `PartnerView`
- [x] `ProductEventTranslator.java`: übersetzt `ProductDefinedPayload` → `ProductView`
- [x] [PartnerEventConsumer.java](policy/src/main/java/ch/css/policy/infrastructure/messaging/PartnerEventConsumer.java) und [ProductEventConsumer.java](policy/src/main/java/ch/css/policy/infrastructure/messaging/ProductEventConsumer.java) delegieren Übersetzung an ACL
- [x] Unit-Tests für beide Translator-Klassen
...
- [x] `quarkus-micrometer-registry-prometheus` in alle drei Service-`pom.xml` hinzufügen
- [x] Prometheus-Service in [docker-compose.yaml](docker-compose.yaml) ergänzen; scraped `/q/metrics` aller Quarkus-Services
- [x] Grafana-Service in docker-compose ergänzen; Port 3000
- [x] Vorkonfiguriertes Grafana-Dashboard als JSON-Datei: Kafka-Consumer-Lag, JVM-Heap, DB-Connection-Pool, HTTP-Request-Rate
- [x] `grafana/provisioning/` in [infra/](infra/) anlegen
...
- [x] [docker-compose.yaml](docker-compose.yaml): Image-Namen auf `css/partner-service`, `css/product-service`, `css/policy-service` umbenennen
- [x] [.env.example](.env.example): `IMAGE_REGISTRY=css` und `IMAGE_TAG=latest` als konfigurierbare Variablen
- [x] Quarkus-Maven-Plugin in den `pom.xml` Dateien: `quarkus.container-image.group=${IMAGE_REGISTRY:css}` konfigurieren
- [x] [build.sh](build.sh): Registry und Tag aus ENV lesen
...
- [x] `infra/platform/init.sql`: Schemas `partner_raw`, `product_raw`, `policy_raw` anlegen statt alles in `public`
- [x] `infra/platform/consumer.py`: Events in das jeweilige Domain-Schema schreiben
- [x] `infra/dbt/`-Modelle: `source`-Definitionen auf Schema-Namespaces aktualisieren

**Langfristig (P3):**
- [x] Separate dbt-Projekte pro Domain: `infra/dbt-partner/`, `infra/dbt-product/`, `infra/dbt-policy/`
...
- [x] GitHub-Actions-/CI-Pipeline anlegen (`.github/workflows/contract-check.yml`):
...
- [x] DataHub-Ingest um Contract-Version aus ODC-`metadata.version`-Feld ergänzen
...
- [x] `billing/specs/business_spec.md` anlegen (Platzhalter: Fakturierung, Mahnwesen, Zahlungseingang)
- [x] `sales/specs/business_spec.md` anlegen (Platzhalter: Angebote, Maklervermittlung)
- [x] `specs/arc42.md`: Abschnitte für Billing und Sales mit Status «Geplant, nicht implementiert» ergänzen
- [x] `kafka-init` in [docker-compose.yaml](docker-compose.yaml): Topic-Namespaces `billing.v1.*` und `sales.v1.*` dokumentarisch reservieren (Kommentare)
...
- [x] `docker-compose.prod.yaml` anlegen mit 3-Broker-Kafka-Konfiguration
- [x] README: Sektion «Production Setup» mit Hinweis auf Mindest-3-Broker-Anforderung
- [x] `replication.factor=3`, `min.insync.replicas=2` in Topic-Konfiguration dokumentieren
...
- [x] Alle ODC-Contracts um `accessPatterns`-Sektion ergänzen
- [x] Sample-Consumer-Properties-Dateien unter `contracts/samples/` in jedem Service anlegen
- [x] Data Product Portal (`infra/portal/main.py`): `/governance`-Route um Access-Pattern-Anzeige erweitern
...
- [x] [partner/specs/business_spec.md](partner/specs/business_spec.md): Abschnitt «Consumer Bootstrap Protocol» ergänzen
- [x] [product/specs/business_spec.md](product/specs/business_spec.md): analog (nach TASK-10)
- [x] [policy/specs/business_spec.md](policy/specs/business_spec.md): Abschnitt «Read-Model Bootstrapping» ergänzen
